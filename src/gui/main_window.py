...
class MainWindow(QMainWindow):
    ...
    def _build_menus(self) -> None:
        ...
        file_menu.addAction(icons.icon("clipboard"), "&Copy to Clipboard\tCtrl+C", lambda: self._clip_copy(False))
        file_menu.addAction(icons.icon("clipboard"), "Cu&t to Clipboard\tCtrl+X", lambda: self._clip_copy(True))
        file_menu.addAction(icons.icon("clipboard"), "&Paste\tCtrl+V", self._clip_paste)
        file_menu.addSeparator()
    ...
    def _on_clipboard_request(self, action: str) -> None:
        if action == "paste":
            self._clip_paste()
        else:
            self._clip_copy(cut=(action == "cut"))

    def _clip_copy(self, cut: bool) -> None:
        """Ctrl+C / Ctrl+X: selected entries onto the system clipboard (Explorer-compatible)."""
        from . import file_clipboard
        entries = self._active_panel_widget.selected_entries()
        if not entries:
            return
        file_clipboard.set_files([e.full_path for e in entries], cut=cut)
        verb = "Cut" if cut else "Copied"
        Toast.show_message(self, f"{verb} {len(entries)} item(s) to clipboard", "info", 1500)

    def _clip_paste(self) -> None:
        ...
        for p in paths:
            if same_folder(p, dest):
                ...
                self._submit(JobSpec(job_type=JobType.COPY, sources=[p], destination=target),
                             f"Copying {Path(p).name}…")
                first_new = first_new or Path(target).name
            else:
                plain.append(p)
        if plain:
